  ...
  return (
    <div style={{ padding: '20px', maxWidth: '800px', margin: 'auto' }}>
      <div style={{ marginBottom: '40px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: '20px' }}>
          ...
          <button onClick={handleAddFitnessLog} style={{ padding: '10px', width: '200px', background: 'red', color: '#fff' }}>Add Fitness Log</button>
        </div>
    ...
    {fitnessLogs.length > 0 ? fitnessLogs.map((log, index) => (
      <div key={index} style={{ background: '#f3f3f3', margin: '10px 0', padding: '10px', borderRadius: '8px' }}>
        ...
        <p>Mood: {log.mood}</p>
        <button onClick={() => handleDeleteFitnessLog(index)} style={{ backgroundColor: 'black', color: 'white', padding: '5px 10px' }}>Delete</button>
      </div>
      ...
      <div style={{ marginBottom: '40px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: '20px' }}>
          ...
          <button onClick={handleAddNutritionLog} style={{ padding: '10px', width: '200px', background: 'red', color: '#fff', border: 'none', borderRadius: '5px' }}>Add Nutrition Log</button>
        </div>
        <div style={{ marginTop: '20px' }}>
    ...
    {nutritionLogs.length > 0 ? nutritionLogs.map((log, index) => (
      <div key={index} style={{ background: '#f3f3f3', margin: '10px 0', padding: '10px', borderRadius: '8px' }}>
        ...
        <button onClick={() => handleDeleteNutritionLog(index)} style={{ backgroundColor: 'black', color: 'white', padding: '5px 10px' }}>Delete</button>
      </div>
